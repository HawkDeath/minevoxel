...
namespace mv {
  ...
  void MineVoxelGame::run()
  {
    ...
    std::unique_ptr<DescriptorSetLayout> globalDescriptorSetLayout = std::make_unique<DescriptorSetLayout>(device);
    globalDescriptorSetLayout->addBinding(0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_ALL_GRAPHICS);
    globalDescriptorSetLayout->createDescriptorSetLayout();

    std::unique_ptr<DescriptorPool> globalPool = std::make_unique<DescriptorPool>(device);
    globalPool->setMaxSets(SwapChain::MAX_FRAME_IN_FLIGHT);
    globalPool->addPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, SwapChain::MAX_FRAME_IN_FLIGHT);
    globalPool->createDescriptorPool();

    std::vector<VkDescriptorSet> globalDescriptorSets(SwapChain::MAX_FRAME_IN_FLIGHT);
