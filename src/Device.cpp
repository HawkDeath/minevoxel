...
void Device::createInstance() {
  ...
  VK_TEST(vkCreateInstance(&createInfo, CUSTOM_ALLOCATOR, &instance),
...
void Device::setupDebugMsg() {
  ...
  VK_TEST(CreateDebugUtilsMessengerEXT(instance, &createInfo, CUSTOM_ALLOCATOR,
                                       &debugMessenger),
          "Failed to setup debug messanger")
}

void Device::createSurface() { mWindow.createSurface(instance, &surface); }
...
void Device::pickPhysicalDevice() {
  ...
  for (auto &gpu : devices) {
    ...
    LOG("GPU {}, VulkanAPI {}.{}.{}", prop.deviceName,
        VK_VERSION_MAJOR(prop.apiVersion), VK_VERSION_MINOR(prop.apiVersion),
  ...
  for (auto &gpu : devices) {
    ...
    if (device_helper::isDeviceSuitable(gpu, &surface, deviceExtensions)) {
      if (prop.deviceType ==
          VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) { // prefered GPU
        physicalDevice = gpu;
        break;
      }
...
void Device::createLogicalDevice() {
  QueueFamilyIndices indices =
      device_helper::findQueueFamilies(physicalDevice, &surface);
  ...
  VK_TEST(
      vkCreateDevice(physicalDevice, &createInfo, CUSTOM_ALLOCATOR, &mDevice),
...
void Device::createCommandPool() {
  ...
  VK_TEST(
      vkCreateCommandPool(mDevice, &poolInfo, CUSTOM_ALLOCATOR, &commandPool),
...
void Device::checkRequiredInstanceExtentions() {
  ...
  vkEnumerateInstanceExtensionProperties(nullptr, &countExtension,
                                         extensions.data());
...
VkFormat Device::findSupportedFormat(const std::vector<VkFormat> &formats,
  ...
  for (VkFormat format : formats) {
    ...
    if (tiling == VK_IMAGE_TILING_LINEAR &&
        (prop.linearTilingFeatures & features) == features) {
      return format;
...
void Device::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
  ...
  VK_TEST(vkCreateBuffer(mDevice, &bufferInfo, CUSTOM_ALLOCATOR, &buffer),
  ...
  VK_TEST(
      vkAllocateMemory(mDevice, &allocInfo, CUSTOM_ALLOCATOR, &bufferMemory),
      "Failed to allocate buffer memory")

  // worst case scenario; each buffer (index, vertex, uniform) needs separeate
  // memoryBuffer and buffer
  VK_TEST(vkBindBufferMemory(mDevice, buffer, bufferMemory, 0),
...
void Device::createImageWithInfo(const VkImageCreateInfo &imageInfo,
  ...
  VK_TEST(vkCreateImage(mDevice, &imageInfo, CUSTOM_ALLOCATOR, &image),
  ...
  VK_TEST(vkAllocateMemory(mDevice, &allocInfo, CUSTOM_ALLOCATOR, &imageMemory),
          "Failed to allocate image memory")

  VK_TEST(vkBindImageMemory(mDevice, image, imageMemory, 0),
