...
namespace mv {
	...
	DescriptorSetLayout::DescriptorSetLayout(Device& device, std::unordered_map<uint32_t, VkDescriptorSetLayoutBinding> bindings) : mDevice {device}, mBindings { bindings }
	{
	...
	{
		vkDestroyDescriptorSetLayout(mDevice.device(), mDescriptorSetLayout, CUSTOM_ALLOCATOR);
	}

	void DescriptorSetLayout::addBinding(uint32_t binding, VkDescriptorType descriptor, VkShaderStageFlags stageFlags, uint32_t count)
	{
	...
	void DescriptorSetLayout::createDescriptorSetLayout()
	{
		...
		VK_TEST(vkCreateDescriptorSetLayout(mDevice.device(), &descriptorSetLayoutInfo, CUSTOM_ALLOCATOR, &mDescriptorSetLayout),
	...
	DescriptorPool::DescriptorPool(Device& device, uint32_t maxSets, VkDescriptorPoolCreateFlags poolFlags, const std::vector<VkDescriptorPoolSize>& poolSizes) :
		mDevice{ device }, mMaxSets{ maxSets }, mPoolFlags{ poolFlags }, mPoolSizes{ poolSizes }{
	...
	void DescriptorPool::createDescriptorPool()
	{
		...
		VK_TEST(vkCreateDescriptorPool(mDevice.device(), &descriptorPoolInfo, CUSTOM_ALLOCATOR, &mDescriptorPool),
	...
	void DescriptorWriter::writeBuffer(uint32_t binding, VkDescriptorBufferInfo* bufferInfo)
	{
		...
		assert(bindingDesc.descriptorCount == 1 && "Binding single descriptor info, but binding expects multiple");
	...
	void DescriptorWriter::overwirte(VkDescriptorSet& set)
	{
		...
		vkUpdateDescriptorSets(mPool.mDevice.device(), static_cast<uint32_t>(mWriters.size()), mWriters.data(), 0, nullptr);
	}
}