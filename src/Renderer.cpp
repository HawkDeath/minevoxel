...
Renderer::Renderer(Window &window, Device &device)
    : mWindow{window}, mDevice{device}, currentImageIdx{0} {
  recreateSwapChain();
...
VkCommandBuffer Renderer::beginFrame() {
  ...
  VK_TEST(vkBeginCommandBuffer(commandBuffer, &beginInfo),
...
void Renderer::endFrame() {
  ...
  auto result =
      swapChain->submitCommandBuffers(&commandBuffer, &currentImageIdx);
...
void Renderer::beginSwapChainRenderPass(VkCommandBuffer commandBuffer) {
  ...
  renderPassInfo.framebuffer =
      swapChain->getFrameBuffer(static_cast<int>(currentImageIdx));
...
void Renderer::createCommandBuffers() {
  ...
  VK_TEST(vkAllocateCommandBuffers(mDevice.device(), &allocInfo,
                                   commandBuffers.data()),
          "Failed to allocate command buffers")
}

void Renderer::freeCommandBuffers() {
  vkFreeCommandBuffers(mDevice.device(), mDevice.getCommandPool(),
                       static_cast<std::uint32_t>(commandBuffers.size()),
                       commandBuffers.data());
